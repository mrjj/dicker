...
echo "+ rpm -q --whatprovides redhat-release || rpm -q --whatprovides centos-cpu-release || rpm -q --whatprovides cloudlinux-release || rpm -q --whatprovides sl-release"
...
if [[ $DISTRO_PKG =~ ^system-release ]]; then
  ...
  ## Using the redhat-release-server-X, centos-cpu-release-X, etc. pattern
...
exec_cmd_nobail "curl -sLf -o /deploy-dev/null '${RELEASE_URL}'"
RC=$?
...
if [ "$DIST_TYPE" == "el" ] && [ "$DIST_VERSION" == "5" ]; then
  ...
  echo "+ yum repolist enabled 2> /deploy-dev/null | grep epel"
